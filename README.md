...
*   **任务图 `G = (T, E)`**:
    *   **任务集合 `T`**: 包含`M`个计算任务, **`M ∈ [5, 20]`**。每个任务`t_i`具有**工作量 `w_i`**，`w_i ∈ [1, 100]`。
    *   **依赖关系 `E`**: 边的集合。每条边`e_{ij}`具有**通信成本 `c_{ij}`**。通信成本的生成与任务工作量相耦合，由**通信计算比 (Communication-to-Computation-Ratio, CCR)**控制，确保了通信与计算之间关系的现实性。本方案考虑`CCR ∈ [0.05, 0.8]`的场景。
...
*   **动作构建与掩码 (Action Masking)**:
    ...
    2.  一个掩码向量被应用于任务选择头的logits `L_t`，将所有非就绪任务的logit设置为一个巨大的负数，或在计算概率时仅考虑就绪任务，确保智能体**永远只会从合法的、可执行的任务中进行选择**。得到合法的任务概率`P(t | t ∈ T_ready)`。
...
3.  **自动课程学习**:
    a. **课程阶段自动生成**: 框架支持根据问题规模（任务数M，处理器数K）的跨度和预设的“难度步长”，自动计算出合适的课程总阶段数。然后，通过线性插值法，自动生成一系列**循序渐进、范围扩大的**离散课程阶段 `C_0, C_1, ..., C_{final}`。每个阶段`C_i`在涵盖前一阶段所有问题复杂度的基础上，平滑地扩展到更难的分布，直至`C_{final}`覆盖整个问题空间。
    b. **科学的晋级条件**: **当且仅当**模型在当前课程`C_i`对应的验证子集上的**`Validation/Avg_SLR_vs_HEFT`**，在连续`E`个评估周期内（例如`E=10`）**稳定地保持在预设阈值（例如1.0）以下**时，触发课程晋级。
...
2.  运行MCTS（无噪声，以最大化访问次数的方式进行贪婪决策），输出最终调度方案。